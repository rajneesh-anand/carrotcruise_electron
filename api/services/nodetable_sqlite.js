...
class NodeTableSqlite {
  ...
  order() {
    ...
    if (this.request.order.length > 0) {
      let dtColumns = NodeTableSqlite.pluck(this.columns, "dt");
  ...
  output(callback) {
    ...
    this.db.all(queryString, (err, results, fields) => {
      if (err) {
          ...
          this.db.all(
            ...
            (err, resultCount, columns) => {
              ...
              let output = {
                ...
                data: NodeTableSqlite.mapData(this.columns, results),
              };
...
module.exports = NodeTableSqlite;
